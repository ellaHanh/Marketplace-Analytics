...
### This is a React component, so you have a few options to run it:

Easiest: Online React Playgrounds

- **CodeSandbox (recommended)**
Go to https://codesandbox.io/
Click "Create Sandbox"
Select "React"
Replace the default code with the content from diagram_maker.md
Save and it will render interactively

- **Stackblitz**
Go to https://stackblitz.com/
Click "Create new" → "React"
Paste the code
Auto-runs in the browser
...
# (Copy everything inside the export default function App() { ... })
...
Below is a code for diagram


The diagram is interactive — click any phase card to expand its full component breakdown. A few things to note:

- **Injectors** branch off between Phase 1 and Phase 2, showing they corrupt the raw data before staging picks it up
- **Table lineage** strip at the bottom traces every table through the pipeline in dependency order
- **Phase 2 detail** shows all 5 signed ledger entry types (including the conditional refund row)
- **Phase 4 detail** shows the JSON report convention and the no-short-circuit rule
- **Phase 5 detail** shows the ephemeral DB / singleton patching approach
